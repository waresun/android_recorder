...
get_touch_device(){
    adb shell getevent -pl|sed -e ':a;N;$!ba;s/\n / /g'|\
        grep 'ABS_MT_TOUCH'|sed '1d'|awk '{print $4}'|tr -d '\011\012\015'
}
...
#     4. sed - 替换 'eventX'
cat $afile |\
    gawk '{print strtonum("0x"$2), strtonum("0x"$3), strtonum("0x"$4)}' |\
...
touchdev_tc=${touchdev//\//\\\/}
sed -i "s/_REPLACE_DEVICE_/${touchdev_tc}/g" $targetcfile
